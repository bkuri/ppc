...
// NewAtLine creates a SrcError with optional line number
func NewAtLine(path, id, msg string, line int) errtypes.SrcError {
	return errtypes.SrcError{Path: path, ID: id, Msg: msg, Line: line}
}

// NewWithHint creates a SrcError with optional hint
func NewWithHint(path, id, msg, hint string) errtypes.SrcError {
	return errtypes.SrcError{Path: path, ID: id, Msg: msg, Hint: hint}
}
